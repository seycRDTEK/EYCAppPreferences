...
public class AppPreferences extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
		...
		if (action.equals("fetchArray")){
			String jsonStringToReturn = "[";
			JSONArray optionsArray = args.getJSONArray (0);
			for (int i=0; i<optionsArray.length(); ++i){
				JSONObject options = optionsArray.getJSONObject(i);
				String key  = options.getString("key");
				String dict = options.optString("dict");
				String dictKey = "";
				String jsonStringOfThisObj = "{";
				if (!"".equals(dict)){
					dictKey = dict + '.' + key;
					jsonStringOfThisObj = jsonStringOfThisObj + "\"dict\":\"" + dict + "\",";
					jsonStringOfThisObj = jsonStringOfThisObj + "\"key\":\"" + key + "\",";
					jsonStringOfThisObj = jsonStringOfThisObj + "\"value\":\"" + fetchValueByKeyForArray(dictKey) + "\"}";
				}
				else{
					jsonStringOfThisObj = jsonStringOfThisObj + "\"key\":\"" + key + "\",";
					jsonStringOfThisObj = jsonStringOfThisObj + "\"value\":\"" + fetchValueByKeyForArray(key) + "\"}";
				}
				jsonStringToReturn = jsonStringToReturn + jsonStringOfThisObj + ",";
			}
			jsonStringToReturn = jsonStringToReturn.substring(0, jsonStringToReturn.length()-1) + "]";
			//Log.d(LOG_TAG, "json return :" + jsonStringToReturn);
			callbackContext.success(jsonStringToReturn);
		}
	...
	//This is added by SEYC
	private String fetchValueByKeyForArray(final String key) {
			...
			if (objClass.equals("java.lang.Integer")) {
				...
				if (sharedPrefs.contains("_" + key + "_type")) {
					...
					try {
						...
						Log.d(LOG_TAG, "fakeArray :" + fakeArray);
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
						return "";
					...
					//Log.d(LOG_TAG, "fakeArray after :" + returnVal);
					// returnVal = new JSONStringer().value((String)obj).toString();
				}

			} else {
				Log.d(LOG_TAG, "unhandled type: " + objClass);
			}
			// JSONObject jsonValue = new JSONObject((Map) obj);
			return returnVal;
		} else {
			Log.d(LOG_TAG, "no value");
			return "";
...
//				Log.d(LOG_TAG, "key value : " + entry.getKey() + " " + entry.getValue());
//			}
			if (sharedPrefs.contains(key)) {
				Object obj = sharedPrefs.getAll().get(key);
				String objClass = obj.getClass().getName();
...
//			}
			...
			//		Log.d(LOG_TAG, "value is: " + nv.toString() + " js type is: " + type + " " + args.toString());
			if (type != null) {
				if (sharedPrefs.contains("_" + key + "_type")) {
					editor.remove("_" + key + "_type");
				}
				if (type.equals("string") ) {
					...
					Log.d(LOG_TAG, "complex thing stored");
				}
