...
- (void)fetchArray:(CDVInvokedUrlCommand*)command
{
    ...
    [self.commandDelegate runInBackground:^{
        NSString* jsonStringToReturn = @"[";
        NSArray* optionsArray = [[command arguments] objectAtIndex:0];
        ...
        for (options in optionsArray)
        {
            ...
            @try {
                ...
                NSLog(@"settingsValue : %@", settingsValue);
                ...
                if (settingsDict) {
                    returnVar = [NSString stringWithFormat:@"{\"dict\":\"%@\",\"key\":\"%@\",\"value\":\"%@\"},", settingsDict, settingsName, returnVar];
                } else {
                    returnVar = [NSString stringWithFormat:@"{\"key\":\"%@\",\"value\":\"%@\"},", settingsName, returnVar];
                }
                jsonStringToReturn = [jsonStringToReturn stringByAppendingString:returnVar];
                //[self.commandDelegate sendPluginResult:result callbackId:[command callbackId]];
            }
            
        }
        jsonStringToReturn = [jsonStringToReturn substringToIndex:[jsonStringToReturn length] - 1];
        jsonStringToReturn = [jsonStringToReturn stringByAppendingString:@"]"];
        NSLog(@"JSONSTRINGTORETURN : %@", jsonStringToReturn);
        result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:jsonStringToReturn];
        [self.commandDelegate sendPluginResult:result callbackId:[command callbackId]];
        //end of loop
    }];
}
...
- (void)store:(CDVInvokedUrlCommand*)command
{
    ...
    //This is removed by SEYC
        ...
        @try {
            ...
            //NSLog(@"Stored : dict-%@, key-%@, value-%@", settingsDict,settingsName,settingsValue);
            [self.commandDelegate sendPluginResult:result callbackId:[command callbackId]];
        }
    //This is removed by SEYC
    //}];
