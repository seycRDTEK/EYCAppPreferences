...
//	added by EYC
	...
	AppPreferences.prototype.fetchArray = platform.fetchArray || function (
			successCallback, errorCallback, jsonObjString
	) {

		var jsonObj = JSON.parse(jsonObjString);

		for (var oneObj in jsonObj){
			var dict = jsonObj[oneObj].dict;
			var key = jsonObj[oneObj].key;
		...
		var execStatus = cordova.exec (
				...
				"AppPreferences", "fetchArray", [jsonObj]
		);
	};
